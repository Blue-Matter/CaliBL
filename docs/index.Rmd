...
Based on empirical recreational catch rate and release rate data, for at least two species of California state fisheries, develop theoretical models that predict release rates given alternative bag limit policies, and incorporate these into the open-source [openMSE](https://openmse.com/) framework for testing management procedures.  
...
Recreational fisheries are a substantial contributor to overall fishery exploitation in a number of California State fisheries. For example, for Northern California Halibut, the recreational fishery constitutes roughly 40% of the catches in recent years. 

A primary management measure for recreational fisheries is individual (per angler) bag limits in addition to boat or trip limits. When evaluating the performance of alternative management policies that vary bag limits it is necessary to account for the impacts on both retained catches and biological risk due to post-release mortality. 
...
Preliminary multi-sex, multifleet operating models were developed for California Halibut using the [openMSE](www.openMSE.com) framework ([CDFW 2023b - California Halibut Enhanced Status Report](https://marinespecies.wildlife.ca.gov/california-halibut/), [CDFW 2023c - California Halibut](https://wildlife.ca.gov/Conservation/Marine/Nearshore#56021327-california-halibut)). To rigorously account for bag limit impacts on the stock and fishery it is necessary to build management procedures that realistically account for these dynamics. 
...
Following projects steps refine these data, the derived bag-limit-release-rate models, and operating models for California Halibut, and then extend the approach to an additional species. 
...
This page, the code and information linked from it, are intended to satisfy Deliverable A. 
...
The key priorities relate to getting confidentiality agreements in place to get location-based Recfin data so that regional bag limits can be related to regional catch rate and release rate data. 
...
Recreational CPUE recorded in the [Recfin](http://wiki.recfin.org/index.php/California_Recreational_Fisheries_Survey) database suggests that trip-level observations are log-normally distributed (see biannually disaggregated data of Figure 1).
...
Figure 1. Distribution of trip-level CPUE in the [Recfin](http://wiki.recfin.org/index.php/California_Recreational_Fisheries_Survey) database. Log-normal distributions (red) were fitted to the data (black) and lognormal mean and standard deviation calculated. Data are disaggregated biannually (For example, 2017_1 is Jan-Jun 2017, 2019_2 is Jul-Dec 2019). 

&nbsp;

#### Fit to data of alternative release rate models

Multiple models were derived that could predict release rate given observed CPUE and the applied bag limit (e.g. 3 fish per-trip-per-person in the case of Northern Halibut):
...
The two parameter log normal distribution model that estimates a constant background rate of voluntary releases (L2P1, red line) cannot fit the relatively low release rates at low observed mean catch rate. 

The three parameters log normal distribution model that estimates voluntary release rates given mean fish below 1 fish per trip and those between 1 fish and the bag limit (3 fish) (L3P1, blue line), did a better job but was consistently below the true observations. 

The two parameter log normal distribution model that estimates a linearly increasing rate of background voluntary releases (L2P2, green line) could not fit the lower release proportions that were observed at lower mean catch rates. 

The three parameters log normal distribution model that estimates linearly increasing rate of background voluntary releases from an intercept (L3P2, orange line) fit the data the best with fewer patterns in residuals (observed - model predicted).  
