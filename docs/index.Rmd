...
5) Specify fishery operating models based on empirical stock assessments (a plausible hypothesis for fishery and population dynamics) and also alternatives for robustness/ sensitivity testing. The SS3 report of the Base assessment run for Northern Halibut is available [here](Assessment/SS_output.html) the model files are available [here](Assessment/SS3_NHalibut.zip). The stock assessment for Black Rockfish is available from the [PFMC](https://www.pcouncil.org/documents/2023/08/black-rockfish-in-california-stock-stock-assessment-executive-summary-post-star-panel.pdf)
...
remotes::install_github('blue-matter/CaliBL')
packageVersion('CaliBL')

```

An small simulation version of the operating models is available to demonstrate how to run the CaliBL code: 
...
```{r saveresults,echo=F, eval=F}
# here you can save these tothe package for use in this document later
save(MSE_halibut,file = "C:/GitHub/CaliBL/data/MSE_halibut.RData")
save(MSE_BRF,file = "C:/GitHub/CaliBL/data/MSE_BRF.RData")
```

```{r setup R package,echo=F, eval=T}

library(CaliBL)
```

The results of these MSE analyses can be presented in a number of ways: 


```{r plotresultsdemo1,echo=T, eval=T}
Splot(MSE_halibut)              
 
```

```{r plotresultsdemo3,echo=T, eval=T}
Splot(MSE_BRF)              
